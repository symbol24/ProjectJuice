    ...
    public class PlatformerCharacter2D : MonoBehaviour
    {
        ...
        public void Move(float move, bool dash, bool jump)
        {
            ...
            if ((m_Grounded || m_AirControl) && m_CanDash)
            {
                ...
                if (move > 0 && !m_Controller.m_FacingRight)
                {
                    // ... flip the player.
                    Flip();
                }
                    // Otherwise if the input is moving the player left and the player is facing right...
                else if (move < 0 && m_Controller.m_FacingRight)
                {
                    // ... flip the player.
                    Flip();
                }
        ...
        private void Dash()
        {
            if (m_CanDash)
            {
                ...
                Vector2 angle = new Vector2(m_Controller.m_XAxis, m_Controller.m_YAxis);
